...
int __correction_sender(struct merkel_tree *tree, int datatype_size, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm) {
...
int __correction_receiver(struct merkel_tree *tree, int datatype_size, XXH64_hash_t *hashes, MPI_Datatype datatype, int source, int tag, MPI_Comm comm) {
  int faulty_node_index;
  MPI_Status status;
...
int __correction_all(struct merkel_tree *tree, int datatype_size, XXH64_hash_t *hashes, MPI_Datatype datatype, int root, MPI_Comm comm) {
...
int MPI_Send_Merkle(const void *buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm) {
  ...
  // create merkel tree from data
  struct merkel_tree *tree = create_merkel_tree(buf, size_bytes, ITEM_COUNT * datatype_size);
  tree_retrieve_critical_paths(tree);
...
int MPI_Bcast_Merkle(void *buffer, int count, MPI_Datatype datatype, int root, MPI_Comm comm) {
  ...
  struct merkel_tree *tree = create_merkel_tree(buffer, size_bytes, ITEM_COUNT * datatype_size);
  tree_retrieve_critical_paths(tree);
...
int MPI_Allgather_Merkle(const void *sendbuf, int sendcount, MPI_Datatype sendtype, void *recvbuf, int recvcount, MPI_Datatype recvtype, MPI_Comm comm) {
  ...
  struct merkel_tree *tree = create_merkel_tree(recvbuf, recv_size_bytes * size, ITEM_COUNT * recv_datatype_size);
  tree_retrieve_critical_paths(tree);

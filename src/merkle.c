 ...
 * OR
 * 
 * git clone https://github.com/raspbeep/merkel-mpi.git
...
struct merkel_tree* create_merkel_tree(const void *data, const int data_size, const int merkel_segment_size) {
  struct merkel_tree *tree = malloc(sizeof(struct merkel_tree));
  if (tree == NULL) {
    printf("Error: Unable to allocate memory for merkel_tree\n");
    return NULL;
  }

  // for number of bytes indivisible by segment size
  tree->n_leaves = (data_size + merkel_segment_size - 1) / merkel_segment_size;
  tree->n_nodes = (tree->n_leaves << 1) - 1;

  tree->root = (struct merkle_node *) malloc(tree->n_nodes * sizeof(struct merkle_node));
  if (tree->root == NULL) {
    printf("Error: Unable to allocate memory for merkel_nodes\n");
    return NULL;
  }
  ...
  for (int i = 0; i < tree->n_nodes; i++) {
    ...
    if (tree->root[i].left == NULL && tree->root[i].right == NULL) {
      ...
      int leaf_size = (leaf_index == tree->n_leaves - 1 && data_size % merkel_segment_size != 0) ? data_size % merkel_segment_size : merkel_segment_size;
      tree->root[i].data_size = leaf_size;
      tree->root[i].data = (void *)((char *)data + leaf_index * merkel_segment_size);
    } else {
      tree->root[i].hash = 0;
    }
  }

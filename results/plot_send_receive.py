...
def parse_mpi_results(filename):
    data = {
        "Buffer Size (bytes)": [],
        "Item Count": [],
        "Send-Recv Avg Time (μs)": [],
        "Send-Recv (Merkle) Avg Time (μs)": []
    }

    # Regular expressions to match the relevant lines
    buffer_and_item_pattern = re.compile(r"Buffer size: (\d+) bytes, item count: (\d+)")
    send_recv_pattern = re.compile(r"Send-Recv\s+Size:\s+\d+ bytes\s+Time:\s+min=\s*[\d.]+ μs avg=\s*([\d.]+) μs")
    send_recv_merkle_pattern = re.compile(r"Send-Recv \(Merkle\)\s+Size:\s+\d+ bytes\s+Time:\s+min=\s*[\d.]+ μs avg=\s*([\d.]+) μs")
    ...
    with open(filename, "r") as file:
        for line in file:
            ...
            if send_recv_match and current_buffer_size and current_item_count:
                ...
                data["Send-Recv Avg Time (μs)"].append(float(send_recv_match.group(1)))
                data["Send-Recv (Merkle) Avg Time (μs)"].append(None)  # Placeholder for Merkle timing
            ...
            if send_recv_merkle_match and current_buffer_size and current_item_count:
                ...
                for i in range(len(data["Buffer Size (bytes)"])-1, -1, -1):
                    if data["Buffer Size (bytes)"][i] == current_buffer_size and data["Item Count"][i] == current_item_count:
                        data["Send-Recv (Merkle) Avg Time (μs)"][i] = float(send_recv_merkle_match.group(1))
                        break
...
filename = "send_receive.txt"  # Replace with your file name
...
for item_count in item_counts:
    ...
    if count == 0:
      ...
      plt.plot(df_item["Buffer Size (bytes)"], df_item["Send-Recv Avg Time (μs)"], 'o-', label=f"Send-Recv Avg Time")
      count += 1
    
    # Plot Send-Recv (Merkle) for each item count
    plt.plot(df_item["Buffer Size (bytes)"], df_item["Send-Recv (Merkle) Avg Time (μs)"], 's-', label=f"Send-Recv (Merkle) Avg Time (Item Count={item_count})")
...
# plt.title("MPI Benchmark Results: Send-Recv vs Send-Recv (Merkle) by Item Count")
plt.legend()
